...
//		Triangle t = new Triangle(new Pointf((float) Math.random() * 200, (float) Math.random() * 200),
...
//		g.drawOval((int) (t.getMid().getX() - t.getR()), (int) (t.getMid().getY() - t.getR()), (int) t.getR() * 2,
//				(int) t.getR() * 2);
//	}
...
//		pointfs[3] = new Pointf(150, 200);
		...
		if (graph._getMid(pointfs, c)) {
			...
			g.drawOval((int) (c.getMid().getX() - c.getR()), (int) (c.getMid().getY() - c.getR()), (int) c.getR() * 2,
					(int) c.getR() * 2);
		}		
	}
}